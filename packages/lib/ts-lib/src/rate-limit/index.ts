...
export class ConstantRefillTokenBucketLimiter {
  ...
  public async check({
    ...
    const refillAmount = Math.floor(
      (now - refilledAt) / (this.refillIntervalSeconds * 1000),
    ); // Time ellapsed over refill interval
    if (refillAmount > 0) {
...
export class Throttler {
  ...
  constructor({
    ...
    if (
      cutoffSeconds &&
      cutoffSeconds < timeoutSeconds[timeoutSeconds.length - 1]
    ) {
      throw new Error(
        "Cutoff seconds needs to be longer than the longest time in timeout seconds",
      );
    } else {
    ...
    if (this.resetType && cutoffMilli && timeDifference >= cutoffMilli) {
      ...
      switch (this.resetType) {
        ...
        case "gradual":
          ...
          if (
            !timeoutIndex &&
            (graceCounter >= this.grace || graceDifference < increments)
          ) {
            reset();
            break;
          }
...
export class FixedRefillTokenBucketLimiter {
  ...
  public async check({
    ...
    const needsRefill =
      now - parseInt(bucket.refilledAt) >= this.refillIntervalSeconds * 1000;
    if (needsRefill) {
      updatedBucket = {
        count: this.max - cost,
        refilledAt: now,
